...
运行 npm run accounts:local，程序自动构建本地版本、启动仅监听 127.0.0.1 的服务，并给出一次性的主账号设置链接。链接 30 分钟有效，只能在本机提交，检查 Host/Origin，最多 10 次尝试；主账号创建后关闭设置入口并跳转登录。密码由用户在本地浏览器填写，无需发送给助手。账号页 http://127.0.0.1:3015/account/，后台 http://127.0.0.1:3015/admin/。数据保存在 .private/accounts.sqlite；再次启动沿用原主账号，不覆盖旧账户。
